 ...
 function (exports, shader, framebuffer, data) {
     ...
     function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
         ...
         for (var i = polygon.length - 1; i > 0; i--) {
             var tmp = i < polygon.length - 1 ? i + 1 : 0;
             startPoint = vertices[polygon[i]];
             endPoint = vertices[polygon[tmp]];
             var d = calcDerivative(Math.floor(startPoint[1]), Math.floor(endPoint[1]));
         ...
         for (var j = 0; j < polygon.length; j++) {
             ...
             startPoint = vertices[polygon[j]];

             // Connect edge to next or to first vertex to close the polygon.
             var next = j < polygon.length - 1 ? j + 1 : 0;
             endPoint = vertices[polygon[next]];
     ...
     function scanlineFillPolygon(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
         ...
         for (var y = 0; y < scanlineIntersection.length; y++) {
             ...
             scanlineIntersection[y].sort(function (a, b) { return a.x - b.x; });
             ...
             for (var i = 0; i < scanlineIntersection[y].length - 1; i += 2) {
                 ...
                 var a = scanlineIntersection[y][i].x;
                 var b = scanlineIntersection[y][i + 1].x;
