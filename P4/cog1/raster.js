...
function(exports, shader, framebuffer, data) {
	...
	//	if (startX == endX && startY == endY) return; //Dont draw points
		...
		while (true) {
		  ...
		  if (startY != yStart)
			  addIntersection(startX, startY, 0, 0, edgeStartVertexIndex, edgeEndVertexIndex, null, null);
        }
	  };
	...
	function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
		...
		for (var i = 0; i < scanlineIntersection.length; i++) {
			...
			drawLineBresenham(pair.minX, i, 0, pair.maxX, i, 0, data.getColorByName("red")
			);
		  });
		}
    }
